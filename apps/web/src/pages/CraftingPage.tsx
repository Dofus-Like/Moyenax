...
function FusionSlot({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (raw) {
      try {
        onDrop(JSON.parse(raw) as InventoryItem);
      } catch {
        /* ignore */
      }
    }
    ...
    >
      {item ? (
        <>
          ...
          <button className="fusion-slot-remove" onClick={onRemove} title="Retirer">
            ✕
          </button>
        </>
      ) : (
...
export function CraftingPage() {
  ...
  const isRecipeCraftable = useCallback(
    (recipe: Recipe) => {
      return Object.entries(recipe.craftCost).every(
        ([resId, qty]) => getAvailableQuantity(resId) >= qty,
      );
    },
  ...
  const fusionError =
    fusionSlot1 && fusionSlot2 && !fusionValid
      ? fusionSlot1.id === fusionSlot2.id
  ...
  const draggableInventory = inventory
    ...
    .filter((i) => {
      ...
      if (
        activeFilter === 'ARMOR' &&
        ['ARMOR_HEAD', 'ARMOR_CHEST', 'ARMOR_LEGS'].includes(i.item.type)
      )
        return true;
      if (
        activeFilter === 'OTHER' &&
        !['WEAPON', 'ARMOR_HEAD', 'ARMOR_CHEST', 'ARMOR_LEGS'].includes(i.item.type)
      )
        return true;
      return false;
    });
  ...
  const filteredCategories = [
    ...
    if (activeFilter === 'ARMOR' && ['ARMOR_HEAD', 'ARMOR_CHEST', 'ARMOR_LEGS'].includes(type))
      return true;
    if (
      activeFilter === 'OTHER' &&
      !['WEAPON', 'ARMOR_HEAD', 'ARMOR_CHEST', 'ARMOR_LEGS'].includes(type)
    )
      return true;
    return false;
  });
                      ...
                      return (
                        <div key={recipe.id} className="recipe-card">
                            ...
                            <div className="cost-list">
                              {Object.entries(recipe.craftCost).map(([resId, qty]) => {
                                ...
                                return (
                                  <div
                                    key={resId}
                                    className={`cost-item ${hasEnough ? 'met' : 'missing'}`}
                                  >
                                    <span className="res-details">
                                      {resVisual?.iconPath ? (
                                        <img
                                          src={resVisual.iconPath}
                                          alt=""
                                          style={{
                                            width: 20,
                                            height: 20,
                          ...
                          <button
                            ...
                            disabled={
                              loading ||
                              Object.entries(recipe.craftCost).some(
                                ([resId, qty]) => getAvailableQuantity(resId) < qty,
                              )
                            }
                          >
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="fusion-inv-info">
                          ...
                          <span className="fusion-inv-meta">
                            Rang {inv.rank} · x{inv.quantity}
                          </span>
                        </div>
                        {isInSlot && <span className="fusion-in-slot-badge">Sélectionné</span>}
                        {!isInSlot && inv.quantity < 2 && (
                          <span className="fusion-low-badge">x1</span>
                        )}
                      </div>
                    );
                  })}
                  ...
                  <div className="fusion-preview">
                    ...
                    <div className="fusion-preview-card">
                      ...
                      <div className="fusion-preview-info">
                        ...
                        <span className={`fusion-rank-badge rank-${resultRank}`}>
                          Rang {resultRank}
                        </span>
                        {boostedStats && (
