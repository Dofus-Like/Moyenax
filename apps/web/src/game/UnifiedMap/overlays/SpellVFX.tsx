...
export function SpellVFX({ type, from, to, onComplete }: SpellVFXProps) {
  ...
  useFrame((state, delta) => {
    ...
    if (kunaiMeshRef.current && type.includes('kunai')) {
        ...
        kunaiMeshRef.current.rotation.z = angle - 0.80;
    }
  });
    ...
    return (
      <group>
        <Billboard ref={meshRef}>
          ...
          {!isFire && <ParticleTrail position={new THREE.Vector3(0,0,0)} color={isKunai ? "#ffffff" : "#cbd5e1"} count={isKunai ? 12 : 15} spread={0.2} />}
        </Billboard>
      </group>
    );
  }
