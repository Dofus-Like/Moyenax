...
export const BoundaryOutline = React.memo(
  ({ tiles, gridSize, color, opacity = 1, width = 2, yOffset = 0.05 }: BoundaryOutlineProps) => {
    const edges = useMemo(() => {
...
export const CombatHighlight = React.memo(
  ({
    x,
    y,
    gridSize,
...
export const CombatHighlightsLayer = React.memo(
  ({
    reachableTiles,
    spellRangeTiles,
    pathTarget,
    ...
    return (
      <group>
        ...
        {pathTarget && (
          <group
            position={[pathTarget.x - gridSize / 2 + 0.5, 0.04, pathTarget.y - gridSize / 2 + 0.5]}
          >
            <mesh rotation={[-Math.PI / 2, 0, 0]}>
              <planeGeometry args={[tileSize, tileSize]} />
              <meshBasicMaterial
