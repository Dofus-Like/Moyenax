...
export const gameSessionApi = {
  ...
  getActiveSession: (config?: import('axios').AxiosRequestConfig) =>
    apiClient.get(`${API_URL}/active`, config),
  ...
  getStreamTicket: (sessionId: string) =>
    apiClient.post<{ ticket: string; expiresIn: number }>(
      `${API_URL}/session/${sessionId}/stream-ticket`,
    ),
  resetSession: () => apiClient.post(`${API_URL}/reset`),
};
