...
export function calculateDamage(
  ...
  const baseDamage =
    spell.damage.min + Math.floor(Math.random() * (spell.damage.max - spell.damage.min + 1));
 ...
 */
export function hasLineOfSight(from: CombatPosition, to: CombatPosition, tiles: Tile[]): boolean {
...
export function canMoveTo(
  ...
  const queue: { x: number; y: number; dist: number }[] = [
    { x: currentPosition.x, y: currentPosition.y, dist: 0 },
  ];
